...
namespace ArcForges.ArcNotes.Core;
...
public static partial class BuildIdentity
{
    ...
    public static JsonObject FromAssembly(Assembly assembly)
    {
        ...
        {
            using var stream = assembly.GetManifestResourceStream("ArcNotes." + name) ?? throw new InvalidOperationException("Missing embedded source: " + name);
        ...
        var inputs = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["src/ArcForges.ArcNotes/packages.lock.json"] = Resource("PackageLock")
        };
        ValidateBuild(build);
    ...
    {
        Require(catalog["schemaVersion"]!.GetValue<int>() == 1 && catalog["owner"]!.GetValue<string>() == "ArcNotes", "Wrong source catalog.");
    ...
    public static void ValidateBuild(JsonObject build)
    {
        ...
        if (kind == "ci")
        {
            ...
            Require(!dirty && RunPattern().IsMatch(run) && attempt > 0 && id == run + "." + attempt.ToString(CultureInfo.InvariantCulture)
                && build["pipelineRun"]!.GetValue<string>() == "https://github.com/ArcForges/ArcNotes/actions/runs/" + run, "Invalid CI identity.");
        }
