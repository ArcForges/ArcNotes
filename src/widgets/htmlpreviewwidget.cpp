...
HtmlPreviewWidgetInternal::HtmlPreviewWidgetInternal(QWidget* parent) : QLiteHtmlWidget(parent) {
    viewport()->setStyleSheet(
        QStringLiteral("background-color: %1;")
            .arg(Utils::Schema::schemaSettings->getBackgroundColor(MarkdownHighlighter::HighlighterState::NoState)
                     .name()));
    ...
    connect(this, &QLiteHtmlWidget::contextMenuRequested, this, &HtmlPreviewWidgetInternal::onContextMenuRequested);
}
...
void HtmlPreviewWidgetInternal::onContextMenuRequested(QPoint pos, const QUrl& linkUrl, const QUrl& imageUrl) {
    ...
    auto* act = new QAction(tr("Copy"), this);
    ...
    if (!linkUrl.isEmpty() && linkUrl.isValid()) {
        auto* copyLinkAction = new QAction(tr("Copy link location"), this);
    ...
    if (selectedItem == copyImagePathAction) {
        qApp->clipboard()->setText(imageUrl.isLocalFile() ? imageUrl.toLocalFile() : imageUrl.toString());
...
void HtmlPreviewWidgetInternal::exportAsHTMLFile() {
    ...
    if (ret == QDialog::Accepted) {
        ...
        if (!fileName.isEmpty()) {
            ...
            file.flush();
            file.close();
            Utils::Misc::openFolderSelect(fileName, QStringLiteral("show-exported-preview-html-in-file-manager"));
        }
    }
}
...
HtmlPreviewWidget::HtmlPreviewWidget(QWidget* parent)
    ...
    const bool darkMode = viewModel != nullptr && viewModel->persistentSetting(QStringLiteral("darkMode")).toBool();
    _searchWidget->setDarkMode(darkMode);

    // Forward signals from internal widget
    connect(_htmlWidget, &HtmlPreviewWidgetInternal::anchorClicked, this, &HtmlPreviewWidget::anchorClicked);
}
...
QLiteHtmlSearchWidget* HtmlPreviewWidget::searchWidget() {
...
void HtmlPreviewWidget::setHtml(const QString& text) {
    ...
    const QString underlineCss = QStringLiteral("<style>u, u * { text-decoration: underline; }</style>");
    ...
    const QString strikeoutCss = QStringLiteral("<style>del, s, del *, s * { text-decoration: line-through; }</style>");
    ...
    const QColor hrColor =
        Utils::Schema::schemaSettings->getForegroundColor(MarkdownHighlighter::HighlighterState::HorizontalRule);
    const QString hrCss =
        QStringLiteral("<style>hr { border: 0; border-top: 1px solid %1; height: 0; }</style>").arg(hrColor.name());
...
void HtmlPreviewWidget::setZoomFactor(qreal scale) {
    _htmlWidget->setZoomFactor(scale);
}

qreal HtmlPreviewWidget::zoomFactor() const {
    return _htmlWidget->zoomFactor();
}

QString HtmlPreviewWidget::selectedText() const {
    return _htmlWidget->selectedText();
}

void HtmlPreviewWidget::setDefaultFont(const QFont& font) {
    _htmlWidget->setDefaultFont(font);
}

QFont HtmlPreviewWidget::defaultFont() const {
    return _htmlWidget->defaultFont();
}

bool HtmlPreviewWidget::findText(const QString& text, QTextDocument::FindFlags flags, bool incremental, bool* wrapped) {
    return _htmlWidget->findText(text, flags, incremental, wrapped);
}

QWidget* HtmlPreviewWidget::viewport() const {
    return _htmlWidget->viewport();
}
