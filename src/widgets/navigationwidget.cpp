...
struct HeadingMatch {
    ...
    [[nodiscard]] bool isValid() const { return MarkdownHighlighter::isHeading(elementType) && !text.isEmpty(); }
};
...
int atxHeadingLevel(const QString& text) {
    ...
    return (headingLevel > 0 && i < text.length() && text.at(i) == QLatin1Char(' ')) ? headingLevel : 0;
}
...
HeadingMatch parseHeadingBlock(const QTextBlock& block) {
    if (!block.isValid()) {
        return {};
    }
    ...
    if (!text.trimmed().isEmpty() && isSetextUnderlineText(block.next().text())) {
        ...
        if (!nextText.isEmpty()) {
            const auto state = nextText.at(0) == QLatin1Char('=') ? MarkdownHighlighter::H1 : MarkdownHighlighter::H2;
            return {text.trimmed(), state};
        }
    }

    const int elementType = block.userState();
    return MarkdownHighlighter::isHeading(elementType) ? HeadingMatch{block.text().trimmed(), elementType}
                                                       : HeadingMatch{};
}
...
NavigationWidget::NavigationWidget(QWidget* parent) : QTreeWidget(parent) {
    ...
    QObject::connect(this, &NavigationWidget::currentItemChanged, this, &NavigationWidget::onCurrentItemChanged);
    ...
    QObject::connect(this, &QTreeWidget::itemExpanded, this, &NavigationWidget::onItemExpandedOrCollapsed);
    QObject::connect(this, &QTreeWidget::itemCollapsed, this, &NavigationWidget::onItemExpandedOrCollapsed);

    // Enable context menu
    setContextMenuPolicy(Qt::CustomContextMenu);
    connect(this, &QTreeWidget::customContextMenuRequested, this, &NavigationWidget::showContextMenu);
}

void NavigationWidget::mousePressEvent(QMouseEvent* event) {
    if ((event != nullptr) && (event->button() == Qt::LeftButton) && event->modifiers().testFlag(Qt::ShiftModifier)) {
...
void NavigationWidget::parse(const QTextDocument* document, int textCursorPosition, int noteId) {
    ...
    if (_currentNoteId >= 0 && _currentNoteId != noteId && _collapsedHeadingsPerNote.contains(_currentNoteId)) {
        saveExpandState();
    }
...
QVector<Node> NavigationWidget::parseDocument(const QTextDocument* const document) {
    ...
    for (int i = 0; i < document->blockCount(); ++i) {
        ...
        const HeadingMatch heading = parseHeadingBlock(block);
        if (!heading.isValid()) {
            continue;
        }
...
int NavigationWidget::headingPositionForCursor(const QTextCursor& cursor) {
    ...
    while (block.isValid()) {
        if (parseHeadingBlock(block).isValid()) {
...
int NavigationWidget::findItemIndexForCursorPosition(int position) const {
    auto fwdIt = std::lower_bound(_navigationTreeNodes.begin(), _navigationTreeNodes.end(), position,
...
QTreeWidgetItem* NavigationWidget::findSuitableParentItem(int elementType) const {
    ...
    return (lastHigherItem == nullptr && elementType > MarkdownHighlighter::H1) ? findSuitableParentItem(elementType)
                                                                                : lastHigherItem;
}
