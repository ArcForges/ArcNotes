 ...
 */
void loadTranslation(QTranslator& translator, const QString& fileName, const QString& directory = QString()) {
...
void loadTranslations(QTranslator* translator, const QString& locale) {
    ...
    loadTranslation(translator[3], appPath + "/../src/languages/ArcNotes_" + locale);
    loadTranslation(translator[4], appPath + "/../languages/ArcNotes_" + locale);
    loadTranslation(translator[5], appPath + "/languages/ArcNotes_" + locale);
    loadTranslation(translator[6], appPath + "/ArcNotes_" + locale);
    loadTranslation(translator[7], "../src/languages/ArcNotes_" + locale);
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    loadTranslation(translator[8], "../share/qt6/translations/ArcNotes_" + locale);
    loadTranslation(translator[9], appPath + "/../share/qt6/translations/ArcNotes_" + locale);
#else
    loadTranslation(translator[8], "../share/qt5/translations/ArcNotes_" + locale);
    loadTranslation(translator[9], appPath + "/../share/qt5/translations/ArcNotes_" + locale);
#endif
    loadTranslation(translator[10], "ArcNotes_" + locale);
    loadTranslation(translator[11], "../share/ArcNotes/translations/ArcNotes_" + locale);
    loadTranslation(translator[12], appPath + "/../share/ArcNotes/translations/ArcNotes_" + locale);
}
...
void setupSystemDarkModeChangeCheck(QObject* context) {
    QObject::connect(QGuiApplication::styleHints(), &QStyleHints::colorSchemeChanged, context,
                     [context](Qt::ColorScheme colorScheme) {
                         ...
                         QTimer::singleShot(0, context, [] { Utils::Gui::doSystemDarkModeCheck(true); });
                     });
}
#endif
...
inline void loadReleaseTranslations(QTranslator* translatorsRelease, const QString& locale) {
    ...
    loadTranslation(translatorsRelease[1], "/usr/share/ArcNotes/translations/ArcNotes_" + locale);
}

/**
 * Function for loading the translations on macOS
 */
inline void loadMacTranslations(QTranslator& translatorOSX, QTranslator& translatorOSX2, QTranslator& translatorOSX3,
                                QTranslator& translatorOSX4, const QString& appPath, const QString& locale) {
    loadTranslation(translatorOSX, appPath + "/../Resources/ArcNotes_" + locale);
    loadTranslation(translatorOSX2, "../Resources/ArcNotes_" + locale);
...
int mainStartupMisc(const QStringList& arguments) {
    ...
    parser.setApplicationDescription("ArcNotes " + QString(VERSION));

    // We don't use parser.addHelpOption(), because it added a --help-all option that didn't work
    const QCommandLineOption helpOption(QStringList() << "h" << "help",
                                        QCoreApplication::translate("main", "Displays help on commandline options."));
    parser.addOption(helpOption);

    const QCommandLineOption portableOption(QStringLiteral("portable"),
    ...
    const QCommandLineOption allowMultipleInstancesOption(
        ...
        QCoreApplication::translate("main",
                                    "Allows multiple instances of ArcNotes to be started "
                                    "even if disallowed in the settings."));
    parser.addOption(allowMultipleInstancesOption);
    const QCommandLineOption clearSettingsOption(QStringLiteral("clear-settings"),
                                                 QCoreApplication::translate("main",
                                                                             "Clears the "
    ...
    const QCommandLineOption actionOption(
        ...
        QCoreApplication::translate("main", "Triggers a menu action after the application was started."), "name");
    parser.addOption(actionOption);

    const QCommandLineOption completionOption(
        QStringList() << "completion",
        QCoreApplication::translate("main", "Generate shell completion code. Supports `fish`, `bash`."), "shell");
    parser.addOption(completionOption);

    allOptions << helpOption << portableOption << dumpSettingsOption << versionOption << allowMultipleInstancesOption
               << clearSettingsOption << sessionOption << actionOption << completionOption;
    ...
    if (parser.isSet(completionOption)) {
        ...
        if (shell == "fish") {
            Utils::Cli::generateFishCompletionScript(allOptions, QStringLiteral("ArcNotes"));
        } else if (shell == "bash") {
            Utils::Cli::generateBashCompletionScript(allOptions, QStringLiteral("ArcNotes"));
            //        } else if (shell == "zsh") {
            //            Utils::Cli::generateZshCompletionScript(allOptions,
            //            QStringLiteral("ArcNotes"));
        } else {
    ...
    if (!systemIconTheme) {
        ...
        if (QIcon::themeName().isEmpty() || internalIconTheme) {
            QIcon::setThemeName(QStringLiteral("breeze-arcnotes"));
        }

        if (Utils::Misc::isDarkModeIconTheme()) {
            QIcon::setThemeName(QStringLiteral("breeze-dark-arcnotes"));
        }
    }
    ...
    if (existingNotesPathNotFound) {
        ...
        const auto noteFolders = noteFolderRepository.findAll();
        for (const auto& noteFolder : noteFolders) {
            dir = QDir(noteFolder.localPath);
            if (dir.exists()) {
                notesPath = noteFolder.localPath;
                noteFolderRepository.setCurrentFolderId(noteFolder.id);
    ...
    if (existingNotesPathNotFound) {
        ...
        notesPath = QFileDialog::getExistingDirectory(
            nullptr, QObject::tr("Please select the folder where your notes will get stored to"), notesPath,
    ...
    // if the notes path is empty or doesn't exist ask for a local notes folder
    if (notesPath.isEmpty() || !dir.exists()) {
        notesPath = QFileDialog::getExistingDirectory(
            nullptr, QObject::tr("Please select the folder where your notes will get stored to"),
            Utils::Misc::defaultNotesPath(), QFileDialog::ShowDirsOnly);
...
void tempLogMessageOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg) {
    ...
    if (type == QtWarningMsg && msg.contains(QStringLiteral("Failed to register with host portal"))) {
        return;
    }
    ...
    auto message =
        QStringLiteral("%1 (%2:%3, %4)").arg(msg, context.file, QString::number(context.line), context.function);
...
inline void setAppProperties(QCoreApplication& app, const QString& release, const QStringList& arguments,
                             bool singleApp, bool snap, bool portable, const QString& action, const QString& session) {
...
int main(int argc, char* argv[]) {
    ...
    NoteHistoryRepository::registerLegacyMetaTypes();
    ...
    QGuiApplication::setDesktopFileName(QStringLiteral("PBE.ArcNotes"));
    ...
    // configured see: https://github.com/pbek/ArcNotes/issues/1284
    if (portable) {
    ...
    QCoreApplication::setApplicationName(QStringLiteral("ArcNotes") + appNameAdd);
    ...
    auto clearDiskSettings = [clearSettings, portable]() {
    ...
    if (settings.value(QStringLiteral("overrideInterfaceScalingFactor")).toBool()) {
        qputenv("QT_SCALE_FACTOR",
                QString::number(settings.value(QStringLiteral("interfaceScalingFactor"), 100).toDouble() / 100, 'f', 1)
                    .toUtf8());
    }
    ...
    if (allowOnlyOneAppInstance) {
        allowOnlyOneAppInstance = settings.value(QStringLiteral("allowOnlyOneAppInstance"), true).toBool();
    }
#endif
    ...
    if (allowOnlyOneAppInstance) {
        ...
        if (app.isSecondary()) {
            qWarning() << QCoreApplication::translate("main",
                                                      "Another instance of ArcNotes was already started! "
        ...
        Application qownNotesApp;
        qownNotesApp.initialize();
        MainWindow* w = qownNotesApp.createMainWindow();
        w->show();
        if (!action.isEmpty()) {
            QTimer::singleShot(0, w, &MainWindow::triggerStartupMenuAction);
        }
        ...
        QObject::connect(&app, &SingleApplication::receivedMessage, [&](quint32 instanceId, QByteArray message) {
            Q_UNUSED(instanceId)
            qDebug() << "receivedMessage - 'message': " << message;
        ...
        QObject::connect(&app, &SingleApplication::instanceStarted, [&] {
            qWarning() << QCoreApplication::translate("main",
                                                      "A second instance of ArcNotes was attempted to be "
                                                      "started!");
