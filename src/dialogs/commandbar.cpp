...
public:
    ...
    void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const override {
        ...
        const QString component =
            QStringLiteral("<span style=\"color: gray;\">") + strs.at(0) + QStringLiteral(": </span>");

        str = component + str;
        SettingsViewModel* viewModel = ViewModelLocator::settingsViewModel();
        bool overrideInterfaceFontSize =
            viewModel != nullptr &&
            viewModel->persistentSetting(QStringLiteral("overrideInterfaceFontSize"), false).toBool();
        if (overrideInterfaceFontSize) {
            str = QStringLiteral("<span style=\"font-size: %1px;\">")
                      .arg(viewModel->persistentSetting(QStringLiteral("interfaceFontSize"), 11).toString()) +
...
CommandBar::CommandBar(QWidget* parent) : QMenu(parent) {
    ...
    auto* delegate = new CommandBarStyleDelegate(this);
    auto* del = new ShortcutStyleDelegate(this);
