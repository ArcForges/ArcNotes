...
void MasterDialog::storeGeometrySettings() const {
    if (SettingsViewModel* viewModel = ViewModelLocator::settingsViewModel()) {
        viewModel->setPersistentSetting(getGeometrySettingKey(), saveGeometry());
    }
}
 ...
 * @return settings key
 */
QString MasterDialog::getGeometrySettingKey() const {
...
void MasterDialog::handleOpenDialog() {
    ...
    QByteArray geometryData =
        viewModel == nullptr ? QByteArray() : viewModel->persistentSetting(getGeometrySettingKey()).toByteArray();
...
void MasterDialog::keyPressEvent(QKeyEvent* keyEvent) {
    if (_ignoreReturnKey && (keyEvent->key() == Qt::Key_Enter || keyEvent->key() == Qt::Key_Return)) {
        return;
    }
