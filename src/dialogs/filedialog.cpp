...
FileDialog::FileDialog(const QString& name) {
    if (!name.isEmpty()) {
        ...
        QString path = viewModel == nullptr ? QString() : viewModel->persistentSetting(_settingsKey).toString();
        QFileInfo fileInfo(path);

        // if there are problems with the directory path use path from any
        // FileDialog
        if (!fileInfo.isDir() || !fileInfo.isReadable()) {
            path = viewModel == nullptr ? QString() : viewModel->persistentSetting(_generalSettingsKey).toString();
        }
...
void FileDialog::storeDirectory(QString path) {
    ...
    viewModel->setPersistentSetting(_settingsKey, path);

    // this is the path for all FileDialog
    viewModel->setPersistentSetting(_generalSettingsKey, path);
}
