...
QString Utils::Schema::Settings::currentSchemaKey() const {
    ...
    return settings
        .value(QStringLiteral("Editor/CurrentSchemaKey"),
               !_defaultSchemaKeysList.empty() ? _defaultSchemaKeysList[0] : QStringLiteral("DefaultSchema"))
        .toString();
}
...
QVariant Utils::Schema::Settings::getSchemaValue(const QString& key, const QVariant& defaultValue,
    ...
    QVariant value = isDefaultSchema ? _defaultSchemaSettings.value(schemaKey + QStringLiteral("/") + key, defaultValue)
                                     : SettingsService().value(schemaKey + QStringLiteral("/") + key, defaultValue);

    if (!value.isValid() && schemaNotSet) {
        QString fallbackSchemaKey =
            isDefaultSchema ? _defaultSchemaSettings.value(schemaKey + QStringLiteral("/FallbackSchema")).toString()
...
QFont Utils::Schema::Settings::getFont(int index, QString schemaKey) const {
    ...
    bool enabled =
        getSchemaValue(textSettingsKey(QStringLiteral("FontEnabled"), index), QVariant(), schemaKey).toBool();
    QFont font = getEditorFont(index);

    // if the font is enabled try to fetch it
    if (enabled) {
        auto overrideFont =
            getSchemaValue(textSettingsKey(QStringLiteral("Font"), index), QVariant(), schemaKey).value<QFont>();
...
QColor Utils::Schema::Settings::getForegroundColor(int index, QString schemaKey) const {
    // get the foreground color
    bool enabled =
        getSchemaValue(textSettingsKey(QStringLiteral("ForegroundColorEnabled"), index), QVariant(), schemaKey)
            .toBool();
    QColor color;

    // if the foreground color is enabled try to fetch it
    if (enabled) {
        color = getSchemaValue(textSettingsKey(QStringLiteral("ForegroundColor"), index), QVariant(), schemaKey)
                    .value<QColor>();
    }
...
QColor Utils::Schema::Settings::getBackgroundColor(int index, QString schemaKey) const {
    // get the foreground color
    bool enabled =
        getSchemaValue(textSettingsKey(QStringLiteral("BackgroundColorEnabled"), index), QVariant(), schemaKey)
            .toBool();
    QColor color;

    // if the foreground color is enabled try to fetch it
    if (enabled) {
        color = getSchemaValue(textSettingsKey(QStringLiteral("BackgroundColor"), index), QVariant(), schemaKey)
                    .value<QColor>();
    }
 ...
 * @param index
 */
void Utils::Schema::Settings::setFormatStyle(MarkdownHighlighter::HighlighterState index, QTextCharFormat& format,
                                             QString schemaKey) const {
    ...
    // https://github.com/pbek/ArcNotes/issues/2338
    ...
    format.setBackground(backgroundColorEnabled ? QBrush(getBackgroundColor(index, schemaKey)) : QBrush());

    // set the bold state
    format.setFontWeight(
        getSchemaValue(Utils::Schema::textSettingsKey(QStringLiteral("Bold"), index), QVariant(), schemaKey).toBool()
            ? QFont::Bold
            : QFont::Normal);

    // set the italic state
    format.setFontItalic(
        getSchemaValue(Utils::Schema::textSettingsKey(QStringLiteral("Italic"), index), QVariant(), schemaKey)
            .toBool());

    // set the underline state
    format.setFontUnderline(
        getSchemaValue(Utils::Schema::textSettingsKey(QStringLiteral("Underline"), index), QVariant(), schemaKey)
            .toBool());
}
...
void Utils::Schema::Settings::adaptFontSize(int index, QFont& font, QString schemaKey) const {
    int adaption = getSchemaValue(textSettingsKey(QStringLiteral("FontSizeAdaption"), index), 100, schemaKey).toInt();
...
QFont Utils::Schema::Settings::getEditorFixedFont() const {
    ...
    SettingsService settings;
    QString fontString = settings.value(QStringLiteral("MainWindow/noteTextEdit.code.font")).toString();
...
QString Utils::Schema::getSchemaStyles() {
    ...
    // inherit the styles correctly (https://github.com/pbek/ArcNotes/issues/3218)
    schemaStyles += encodeCssStyleForState(
        MarkdownHighlighter::Bold,
        QStringLiteral("b, strong, i b, em b, i strong, em strong, b i, strong i, b em, strong em"));
    schemaStyles +=
        encodeCssStyleForState(MarkdownHighlighter::Italic,
    ...
    // https://github.com/pbek/ArcNotes/issues/2669
    if (SettingsService().value(QStringLiteral("fullyHighlightedBlockquotes")).toBool()) {
        schemaStyles += encodeCssStyleForState(MarkdownHighlighter::BlockQuote, QStringLiteral("blockquote"));
...
QString Utils::Schema::encodeCssStyleForState(MarkdownHighlighter::HighlighterState index, const QString& htmlTag) {
    ...
    // https://github.com/pbek/ArcNotes/issues/3218
