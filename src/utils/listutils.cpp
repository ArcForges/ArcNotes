...
ListLineData parseListLine(const QString& line) {
    ListLineData data;
    ...
    static const QRegularExpression listRegex(
        QStringLiteral(R"(^(\s*)(?:([+\-*])|(\d+[\.)])|([A-Za-z]+[\.)]))(?:\s\[( |x|X|-)\])?\s+(.*)$)"));
...
QString checkboxPrefix(const QString& marker, QChar state) {
    return marker + QStringLiteral(" [") + state + QStringLiteral("] ");
}

QString alphabeticalListLabel(int index) {
    QString label;
    int value = index;
...
int checkboxStateRank(QChar state) {
...
bool checkboxStateForLine(const QString& line, QChar& state) {
...
void addCheckboxNode(QList<CheckboxTreeNode>& level, QString row, const QString& singleIndentation) {
...
void sortCheckboxLevel(QList<CheckboxTreeNode>& level) {
    for (auto& item : level) {
        sortCheckboxLevel(item.children);
    }

    std::stable_sort(level.begin(), level.end(), [](const CheckboxTreeNode& a, const CheckboxTreeNode& b) {
        QChar stateA;
        QChar stateB;
...
void appendCheckboxLevelText(const QList<CheckboxTreeNode>& level, const QString& topLevelIndentation,
                             const QString& singleIndentation, const QString& prefix, QStringList& output) {
    for (const auto& item : level) {
        output.append(topLevelIndentation + prefix + item.text);
        if (!item.children.isEmpty()) {
            appendCheckboxLevelText(item.children, topLevelIndentation, singleIndentation, prefix + singleIndentation,
                                    output);
        }
    }
}

QStringList selectedTextLines(const QString& text) {
...
QString Utils::ListUtils::createAlphabeticalList(const QString& text) {
    ...
    for (QString& line : lines) {
        ...
        const auto data = parseListLine(line);
        line = data.indentation + alphabeticalListLabel(index++) + QStringLiteral(". ") + data.content;
    }
...
QString Utils::ListUtils::createCheckboxList(const QString& text) {
    ...
    for (QString& line : lines) {
        ...
        const auto data = parseListLine(line);
        line = data.indentation + checkboxPrefix(QStringLiteral("-"), QLatin1Char(' ')) + data.content;
    }
...
QString Utils::ListUtils::orderCheckboxes(const QString& text) {
    ...
    appendCheckboxLevelText(structuredRows, topLevelIndentation, singleIndentation, QString(), orderedRows);
