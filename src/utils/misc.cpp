...
public:
    ...
    [[nodiscard]] QString anonymize(QString text) const {
...
QString anonymizedDebugInformationValue(const QVariant& value) {
    ...
    switch (value.type()) {
        ...
        case QMetaType::QStringList:
            return QStringLiteral("<anonymized string list with %1 item(s)>").arg(value.toStringList().count());
        case QMetaType::QVariantList:
            return QStringLiteral("<anonymized variant list with %1 item(s)>").arg(value.toList().count());
...
bool isAnonymizedDebugInformationKey(const QString& key) {
    return key.startsWith(QStringLiteral("savedSearches/")) || (key == QStringLiteral("recentNoteFolders"));
}

bool isHiddenDebugEnvironmentKey(const QString& key) {
    static const QRegularExpression secretKeyRegex(
        QStringLiteral(R"((^|_)(TOKEN|SECRET|PASSWORD|PASS|API_KEY|AUTH|COOKIE|CREDENTIALS?)(_|$))"),
 ...
 * (thank you to qBittorrent for the inspiration)
 */
void Utils::Misc::openFolderSelect(const QString& absolutePath, const QString& questionDialogIdentifier) {
    if (!questionDialogIdentifier.isEmpty() &&
        Utils::Gui::questionNoSkipOverride(
            QApplication::activeWindow(), QCoreApplication::translate("Utils::Misc", "Open folder in file manager"),
            QCoreApplication::translate("Utils::Misc", "Do you want to show this item in your file manager?"),
            questionDialogIdentifier, QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes) != QMessageBox::Yes) {
        return;
    }
    ...
    if (QFileInfo(path).exists()) {
        ...
        QString cmd = QStringLiteral("explorer.exe /select,\"%1\"").arg(QDir::toNativeSeparators(absolutePath));
        auto* lpCmd = new WCHAR[cmd.size() + 1];
        cmd.toWCharArray(lpCmd);
        lpCmd[cmd.size()] = 0;

        bool ret =
            ::CreateProcessW(nullptr, lpCmd, nullptr, nullptr, FALSE, 0, nullptr, nullptr, &startupInfo, &processInfo);
        delete[] lpCmd;
    ...
    if (!fullXdgMimePath.isEmpty() && QFileInfo(path).exists()) {
        ...
        if (output == QStringLiteral("dolphin.desktop") || output == QStringLiteral("org.kde.dolphin.desktop")) {
            proc.startDetached(QStringLiteral("dolphin"), QStringList{"--select", QDir::toNativeSeparators(path)});
            ...
            proc.startDetached(QStringLiteral("nautilus"), QStringList{"--no-desktop", QDir::toNativeSeparators(path)});
        } else if (output == QStringLiteral("caja-folder-handler.desktop")) {
            const QDir dir = QFileInfo(path).absoluteDir();
            const QString absDir = dir.absolutePath();
            proc.startDetached(QStringLiteral("caja"), QStringList{"--no-desktop", QDir::toNativeSeparators(absDir)});
        } else if (output == QStringLiteral("nemo.desktop")) {
            proc.startDetached(QStringLiteral("nemo"), QStringList{"--no-desktop", QDir::toNativeSeparators(path)});
        } else if (output == QStringLiteral("konqueror.desktop") || output == QStringLiteral("kfmclient_dir.desktop")) {
            proc.startDetached(QStringLiteral("konqueror"), QStringList{"--select", QDir::toNativeSeparators(path)});
        } else {
...
QString Utils::Misc::removeIfStartsWith(QString text, const QString& removeString) {
    if (text.startsWith(removeString)) {
        text.remove(QRegularExpression(QStringLiteral("^") + QRegularExpression::escape(removeString)));
    }
...
QString Utils::Misc::removeIfEndsWith(QString text, const QString& removeString) {
    if (text.endsWith(removeString)) {
        text.remove(QRegularExpression(QRegularExpression::escape(removeString) + QStringLiteral("$")));
    }
...
QString Utils::Misc::toSentenceCase(const QString& text) {
    ...
    //     whitespace)
    static const QRegularExpression sentenceSplitter(QStringLiteral(R"((^[\s\v]*|[.?!][\s\v]+)\K)"));
...
#ifdef Q_OS_MAC
    return process.startDetached(QStringLiteral("open"), QStringList() << executablePath << "--args" << parameters,
                                 workingDirectory);
#else
...
bool Utils::Misc::openFilesWithApplication(const QString& executablePath, const QStringList& files,
    ...
    // start executablePath detached with parameters
    return process.startDetached(QStringLiteral("open"), QStringList() << "-a" << executablePath << files,
                                 workingDirectory);
#else
 ...
 */
QByteArray Utils::Misc::startSynchronousProcess(const QString& executablePath, const QStringList& parameters,
                                                const QByteArray& data, const QString& workingDirectory) {
    QProcess process;
...
QString Utils::Misc::defaultNotesPath() {
    ...
    QString path = isInPortableMode() ? portableDataPath() : QDir::homePath();

    // add "Notes" to the base path
    path += Utils::Misc::dirSeparator() % QStringLiteral("Notes");

    // remove the snap path for Snapcraft builds
    static const QRegularExpression re(QStringLiteral(R"(snap\/arcnotes\/\w\d+\/)"));
    path.remove(re);
 ...
 * @return
 */
char Utils::Misc::dirSeparator() {
    return '/';
}

void Utils::Misc::waitMsecs(int msecs) {
    QTime dieTime = QTime::currentTime().addMSecs(msecs);
    while (QTime::currentTime() < dieTime) QCoreApplication::processEvents(QEventLoop::AllEvents, 50);
}
...
QString Utils::Misc::portableDataPath(const QString& earlyArgv0Path) {
    ...
    if (qApp != nullptr) {
        ...
        // (see https://github.com/pbek/ArcNotes/issues/3542)
...
QString Utils::Misc::htmlToMarkdown(QString text) {
    ...
    text.replace(
        QRegularExpression(QStringLiteral("<(s|strike|del).*?>(.+?)<\\/\\1>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("~~\\2~~"));

    // Handle underline (convert to emphasis since Markdown doesn't have native underline)
    text.replace(
        QRegularExpression(QStringLiteral("<u.*?>(.+?)<\\/u>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("_\\1_"));

    // Handle mark/highlight
    text.replace(
        QRegularExpression(QStringLiteral("<mark.*?>(.+?)<\\/mark>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("==\\1=="));

    // Handle subscript and superscript
    text.replace(
        QRegularExpression(QStringLiteral("<sub.*?>(.+?)<\\/sub>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("~\\1~"));
    text.replace(
        QRegularExpression(QStringLiteral("<sup.*?>(.+?)<\\/sup>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("^\\1^"));

    // Handle blockquote
    text.replace(
        QRegularExpression(QStringLiteral("<blockquote.*?>(.+?)<\\/blockquote>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("\n\n> \\1\n\n"));

    // Handle horizontal rule
    text.replace(QRegularExpression(QStringLiteral("<hr.*?/?>"), QRegularExpression::CaseInsensitiveOption),
                 QStringLiteral("\n\n---\n\n"));
    ...
    text.replace(
        QRegularExpression(QStringLiteral("<pre[^>]*><code[^>]+class=\"[^\"]*language-([^"
                                          "\"\\s]+)[^\"]*\"[^>]*>(.+?)<\\/code><\\/pre>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("\n\n```\\1\n\\2\n```\n\n"));
    ...
    text.replace(
        QRegularExpression(QStringLiteral("<ol[^>]*?>(.*?)<\\/ol>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("\\1"));

    // Handle unordered lists
    text.replace(
        QRegularExpression(QStringLiteral("<ul[^>]*?>(.*?)<\\/ul>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("\\1"));
    ...
    text.replace(QRegularExpression(QStringLiteral("<br.*?>"), QRegularExpression::CaseInsensitiveOption),
                 QStringLiteral("\n"));

    // Handle links (must be after images)
    text.replace(
        QRegularExpression(QStringLiteral("<a[^>]+href=\"(.+?)\".*?>(.+?)<\\/a>"),
                           QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption),
        QStringLiteral("[\\2](\\1)"));
    ...
    text.remove(QRegularExpression(QStringLiteral("<[^>]+>"), QRegularExpression::CaseInsensitiveOption));
...
QString Utils::Misc::parseTaskList(const QString& html, bool clickable) {
    ...
    const auto stripTaskListParagraphs = [](QString& input) {
        ...
        while (true) {
            ...
            content.replace(
                QRegularExpression(QStringLiteral(R"(<\s*/?\s*p[^>]*>)"), QRegularExpression::CaseInsensitiveOption),
                QString());
    ...
    text.replace(re3, listTag % QStringLiteral("\\1&#10005;"));
    text.replace(re1, listTag % QStringLiteral("\\1") % checkboxStart % QStringLiteral("\">&#9744;</a>"));
    text.replace(re2, listTag % QStringLiteral("\\1") % checkboxStart % QStringLiteral("\">&#9745;</a>"));
...
QString Utils::Misc::logFilePath() {
    return appDataPath() % QStringLiteral("/ArcNotes.log");
}
...
void Utils::Misc::restartApplication() {
    ...
    // current instance
    if (qApp->property("singleApplication").toBool() && !parameters.contains("--allow-multiple-instances")) {
...
QString Utils::Misc::applicationPath() {
    ...
    if (isAppImage()) {
        ...
        // (see https://github.com/pbek/ArcNotes/issues/2728#issuecomment-1466522381)
...
QByteArray Utils::Misc::friendlyUserAgentString() {
    const auto pattern = QStringLiteral("%1 (ArcNotes - %2)");
...
QByteArray Utils::Misc::downloadUrlWithStatusCode(const QUrl& url, int& returnStatusCode, bool usePost,
                                                  QByteArray postData) {
    ...
    networkRequest.setHeader(QNetworkRequest::UserAgentHeader, Utils::Misc::friendlyUserAgentString());
...
QHash<int, Utils::Misc::SearchEngine> Utils::Misc::getSearchEnginesHashMap() {
    ...
    searchEngines.insert(
        ...
        {QStringLiteral("Google"), QStringLiteral("https://www.google.com/search?q="), SearchEngines::Google});
    searchEngines.insert(SearchEngines::Bing, {QStringLiteral("Bing"), QStringLiteral("https://www.bing.com/search?q="),
                                               SearchEngines::Bing});
    searchEngines.insert(SearchEngines::DuckDuckGo,
                         {QStringLiteral("DuckDuckGo"), QStringLiteral("https://duckduckgo.com/?t=arcnotes&q="),
                          SearchEngines::DuckDuckGo});
    searchEngines.insert(
        SearchEngines::Yahoo,
        {QStringLiteral("Yahoo"), QStringLiteral("https://search.yahoo.com/search?p="), SearchEngines::Yahoo});
    searchEngines.insert(SearchEngines::GoogleScholar,
                         {QStringLiteral("Google Scholar"), QStringLiteral("https://scholar.google.co.il/scholar?q="),
                          SearchEngines::GoogleScholar});
    searchEngines.insert(
        SearchEngines::Yandex,
        {QStringLiteral("Yandex"), QStringLiteral("https://www.yandex.com/search/?text="), SearchEngines::Yandex});
    searchEngines.insert(
        SearchEngines::AskDotCom,
        {QStringLiteral("Ask.com"), QStringLiteral("https://www.ask.com/web?q="), SearchEngines::AskDotCom});
    searchEngines.insert(SearchEngines::Qwant,
                         {QStringLiteral("Qwant"), QStringLiteral("https://www.qwant.com/?q="), SearchEngines::Qwant});
...
QList<int> Utils::Misc::getSearchEnginesIds() {
    return QList<int>{SearchEngines::DuckDuckGo, SearchEngines::Google,        SearchEngines::Bing,
...
int Utils::Misc::compareVersions(const QString& left, const QString& right) {
    static const QRegularExpression separator(QStringLiteral("[^0-9A-Za-z]+"));
    const QStringList leftParts = left.split(separator, Qt::SkipEmptyParts);
    const QStringList rightParts = right.split(separator, Qt::SkipEmptyParts);
    const int partCount = std::max(leftParts.count(), rightParts.count());

    for (int i = 0; i < partCount; ++i) {
        const QString leftPart = i < leftParts.count() ? leftParts.at(i) : QString();
        const QString rightPart = i < rightParts.count() ? rightParts.at(i) : QString();
        ...
        const int textComparison = QString::compare(leftPart, rightPart, Qt::CaseInsensitive);
        if (textComparison != 0) {
...
bool Utils::Misc::useInternalExportStylingForPreview() {
    return SettingsService().value(QStringLiteral("MainWindow/noteTextView.useInternalExportStyling"), true).toBool();
}
...
bool Utils::Misc::isPreviewUseEditorStyles() {
    return SettingsService().value(QStringLiteral("MainWindow/noteTextView.useEditorStyles"), true).toBool();
}
...
QString Utils::Misc::indentCharacters() {
    return SettingsService().value("Editor/useTabIndent").toBool() ? QStringLiteral("\t")
...
int Utils::Misc::indentSize() {
    return SettingsService().value("Editor/indentSize", 4).toInt();
}
 ...
 * @param data
 */
QString Utils::Misc::prepareDebugInformationLine(const QString& headline, QString data, bool withGitHubLineBreaks,
...
QString Utils::Misc::generateDebugInformation(bool withGitHubLineBreaks, bool anonymize) {
    const SettingsService settings;
    const NoteFolderRepository noteFolderRepository;
    const NoteRepository noteRepository;
    const NoteSubFolderRepository noteSubFolderRepository;
    const QList<NoteFolderData> noteFolders = noteFolderRepository.findAll();
    DebugInformationAnonymizer anonymizer;
    QString output;

    if (anonymize) {
        const QStringList homePaths = {QDir::homePath(), environmentVariableValue("HOME"),
                                       environmentVariableValue("USERPROFILE"), environmentVariableValue("HOMEPATH")};
    ...
    output += QStringLiteral("ArcNotes Debug Information\n");
    output += QStringLiteral("===========================\n");

    QDateTime dateTime = QDateTime::currentDateTime();

    // add information about ArcNotes
    output += QStringLiteral("\n## General Info\n\n");
    output += prepareDebugInformationLine(QStringLiteral("Current Date"), dateTime.toString(), withGitHubLineBreaks);
    ...
    output += prepareDebugInformationLine(QStringLiteral("Build number"), QString::number(BUILD), withGitHubLineBreaks);
    output += prepareDebugInformationLine(QStringLiteral("Platform"),
                                          QStringLiteral("%1 (%2)").arg(PLATFORM, QGuiApplication::platformName()),
                                          withGitHubLineBreaks);

#if (QT_VERSION >= QT_VERSION_CHECK(5, 4, 0))
    output += prepareDebugInformationLine(QStringLiteral("Operating System"), QSysInfo::prettyProductName(),
                                          withGitHubLineBreaks);
    output += prepareDebugInformationLine(QStringLiteral("Build architecture"), QSysInfo::buildCpuArchitecture(),
                                          withGitHubLineBreaks);
    output += prepareDebugInformationLine(QStringLiteral("Current architecture"), QSysInfo::currentCpuArchitecture(),
                                          withGitHubLineBreaks);
#endif
    output += prepareDebugInformationLine(QStringLiteral("Release"), qApp->property("release").toString(),
                                          withGitHubLineBreaks);
    output += prepareDebugInformationLine(QStringLiteral("Qt Version (build)"), QStringLiteral(QT_VERSION_STR),
                                          withGitHubLineBreaks);
    ...
    output += prepareDebugInformationLine(
        QStringLiteral("Portable mode"), Utils::Misc::isInPortableMode() ? QStringLiteral("yes") : QStringLiteral("no"),
        withGitHubLineBreaks);
    output +=
        prepareDebugInformationLine(QStringLiteral("Settings path / key"), settings.fileName(), withGitHubLineBreaks);
    ...
    output += prepareDebugInformationLine(QStringLiteral("QLiteHtml"), useQLiteHtml, withGitHubLineBreaks);

    output +=
        prepareDebugInformationLine(QStringLiteral("Locale (system)"), QLocale::system().name(), withGitHubLineBreaks);
    ...
    if (primaryScreen != nullptr) {
        ...
        output += prepareDebugInformationLine(QStringLiteral("Primary screen resolution"), screenResolution,
                                              withGitHubLineBreaks);
    }
    ...
    output += prepareDebugInformationLine(QStringLiteral("Icon theme"), QIcon::themeName(), withGitHubLineBreaks);
    output += prepareDebugInformationLine(QStringLiteral("Notes in current note folder"),
                                          QString::number(noteRepository.countAll()), withGitHubLineBreaks);
    output += prepareDebugInformationLine(QStringLiteral("Database drivers"),
                                          QSqlDatabase::drivers().join(QStringLiteral(", ")), withGitHubLineBreaks);

    // add note folder information
    output += QStringLiteral("\n## Note folders\n\n");
    output +=
        prepareDebugInformationLine(QStringLiteral("currentNoteFolderId"),
                                    QString::number(noteFolderRepository.currentFolderId()), withGitHubLineBreaks);

    if (noteFolders.count() > 0) {
        Q_FOREACH (const NoteFolderData& noteFolder, noteFolders) {
            const QString noteFolderId = QString::number(noteFolder.id);
            const NoteSubFolderData activeNoteSubFolder =
                noteSubFolderRepository.findByPathData(noteFolder.activeNoteSubFolderData);

            const QString noteFolderName =
                anonymize ? QStringLiteral("note-folder-%1").arg(noteFolderId) : noteFolder.name;

            output += QStringLiteral("\n### Note folder `") % noteFolderName + QStringLiteral("`\n\n");
            output +=
                prepareDebugInformationLine(QStringLiteral("id"), QString::number(noteFolder.id), withGitHubLineBreaks);
            output += prepareDebugInformationLine(
                QStringLiteral("isCurrent"),
                noteFolder.id == noteFolderRepository.currentFolderId() ? QStringLiteral("yes") : QStringLiteral("no"),
                withGitHubLineBreaks);
            output += prepareDebugInformationLine(QStringLiteral("activeTagId"),
                                                  QString::number(noteFolder.activeTagId), withGitHubLineBreaks);
            output += prepareDebugInformationLine(QStringLiteral("localPath"),
                                                  QDir::toNativeSeparators(noteFolder.localPath), withGitHubLineBreaks);
            output += prepareDebugInformationLine(
                QStringLiteral("isShowSubfolders"),
                noteFolder.showSubfolders ? QStringLiteral("yes") : QStringLiteral("no"), withGitHubLineBreaks);
            output += prepareDebugInformationLine(
                QStringLiteral("allowDifferentNoteFileName"),
                noteFolderRepository.settingValue(noteFolder.id, QStringLiteral("allowDifferentNoteFileName")).toBool()
                    ? QStringLiteral("yes")
                    : QStringLiteral("no"),
                withGitHubLineBreaks);
            output += prepareDebugInformationLine(
                QStringLiteral("activeNoteSubFolder name"),
                activeNoteSubFolder.name.isEmpty()
                    ? QString()
                    : (anonymize ? QStringLiteral("note-subfolder") : activeNoteSubFolder.name),
                withGitHubLineBreaks);
            output += prepareDebugInformationLine(
                QStringLiteral("database file"),
                QDir::toNativeSeparators(noteFolder.localPath + QStringLiteral("/notes.sqlite")), withGitHubLineBreaks);
        }
    }
    ...
    QStringList keyHiddenList = {"networking/proxyPassword"};
    ...
    while (itr.hasNext()) {
        ...
        if (keyHiddenList.contains(key)) {
            output +=
                prepareDebugInformationLine(key, QStringLiteral("<hidden>"), withGitHubLineBreaks, value.typeName());
        } else if (anonymize && isAnonymizedDebugInformationKey(key)) {
            output += prepareDebugInformationLine(key, anonymizedDebugInformationValue(value), withGitHubLineBreaks,
                                                  value.typeName());
        } else {
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
            switch (value.typeId()) {
...
#endif
                case QMetaType::QStringList:
                    output += prepareDebugInformationLine(key, value.toStringList().join(QStringLiteral(", ")),
                                                          withGitHubLineBreaks, value.typeName());
                    break;
                case QMetaType::QVariantList:
                    output += prepareDebugInformationLine(
                        key, QStringLiteral("<variant list with %1 item(s)>").arg(value.toList().count()),
                        withGitHubLineBreaks, value.typeName());
                    break;
                case QMetaType::QByteArray:
                case QMetaType::User:
                    output += prepareDebugInformationLine(key, QStringLiteral("<binary data>"), withGitHubLineBreaks,
                                                          value.typeName());
                    break;
                default:
                    output +=
                        prepareDebugInformationLine(key, value.toString(), withGitHubLineBreaks, value.typeName());
            }
        }
    }
    ...
    while (itr.hasNext()) {
        ...
        output += prepareDebugInformationLine(
            key, anonymize ? anonymizedDebugEnvironmentValue(key, value, anonymizer) : value, withGitHubLineBreaks);
    }
...
bool Utils::Misc::regExpInListMatches(const QString& text, const QStringList& regExpList) {
    return std::ranges::any_of(regExpList, [&text](const QString& regExp) {
        const QString trimmed = regExp.trimmed();
        return !trimmed.isEmpty() && QRegularExpression(trimmed).match(text).hasMatch();
    });
}
 ...
 * @param html
 */
void Utils::Misc::transformRemotePreviewImages(QString& html, int maxImageWidth, ExternalImageHash* externalImageHash) {
...
QString Utils::Misc::generateRandomString(int length) {
    const QString possibleCharacters(QStringLiteral("ABCDEFGHKLMNPQRSTUVWXYZabcdefghkmnpqrstuvwxyz23456789"));
...
/**
 * @brief Removes accelerator markers from a label
 *
...
void Utils::Misc::switchToDarkOrLightMode(bool darkMode) {
    ...
    const ColorModeRepository colorModeRepository;
    colorModeRepository.ensureBuiltInModesExist();
    const QString modeId = darkMode ? colorModeRepository.darkModeId() : colorModeRepository.lightModeId();
    const ColorModeData mode = colorModeRepository.findById(modeId);
    colorModeRepository.setCurrentId(modeId);

    // Apply the color mode settings to the global settings
    const SettingsRepository settings;
    settings.setValue("darkMode", mode.darkMode);
    settings.setValue("darkModeColors", mode.darkModeColors);
    settings.setValue("darkModeIconTheme", mode.darkModeIconTheme);
    settings.setValue("darkModeTrayIcon", mode.darkModeTrayIcon);
    settings.setValue("internalIconTheme", mode.internalIconTheme);
    settings.setValue("systemIconTheme", mode.systemIconTheme);

    if (!mode.editorColorSchemaKey.isEmpty()) {
        settings.setValue("Editor/CurrentSchemaKey", mode.editorColorSchemaKey);
    }
}
    ...
    if (logFile.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
        ...
        QString text =
            QStringLiteral("[%1] [%2]: %3\n")
                .arg(dateTime.toString(QStringLiteral("MMM dd hh:mm:ss")).remove(QStringLiteral(".")), typeStr, msg);
        out << text;
        logFile.close();
    }
}
...
int levenshteinDistance(const QString& source, const QString& target) {
    ...
    for (int i = 0; i < sourceCount; i++) {
        ...
        for (int j = 0; j < targetCount; j++) {
            column[j + 1] = std::min<int>({1 + column.at(j), 1 + previousColumn.at(1 + j),
...
bool Utils::Misc::isSimilar(const QString& str1, const QString& str2, int threshold) {
    ...
    if ((str1.contains(str2) || str2.contains(str1)) && (abs(str1.length() - str2.length()) <= threshold)) {
        return true;
    }
...
int Utils::Misc::getPreviewRefreshDebounceTime() {
    return SettingsService().value(QStringLiteral("MainWindow/noteTextView.refreshDebounceTime"), 600).toInt();
}
...
QString Utils::Misc::encodeFilePath(const QString& filePath) {
    ...
    for (QString& segment : segments) {
        ...
        segment = QUrl::toPercentEncoding(segment, "/");
    }
 ...
 * This ensures consistent newline handling across
