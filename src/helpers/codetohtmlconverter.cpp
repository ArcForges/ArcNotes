        ...
        switch (input.at(i + 1).toLatin1()) {
            ...
            case 'p':
                if (_currentLang == CodeCSS && input.at(i + 2) == QLatin1Char('x'))
                    if ((i + 3 < input.length()) &&
                        (input.at(i + 3) == QLatin1Char(';') || input.at(i + 3) == QLatin1Char('\n')))
                        isPostAllowed = true;
                break;
            case 'e':
...
int CodeToHtmlConverter::highlightStringLiterals(StringView input, QChar strType, QString& output, int i) {
    int start = i;
    ++i;
                ...
                case '7': {
                    if (i + 4 <= input.length()) {
                        const bool isCurrentOctal =
                            (isOctal(input.at(i + 2).toLatin1()) && isOctal(input.at(i + 3).toLatin1())) ? true : false;
                ...
                case 'x': {
                    if (i + 3 <= input.length()) {
                        const bool isCurrentHex =
                            (isHex(input.at(i + 2).toLatin1()) && isHex(input.at(i + 3).toLatin1())) ? true : false;
...
int CodeToHtmlConverter::highlightComment(StringView input, QString& output, int i, bool isSingleLine) {
    int endPos = -1;
    if (isSingleLine) {
    ...
    if (commentEndPos >= 0) {
        ...
        // See: https://github.com/pbek/ArcNotes/issues/2959
        return i;
    };
...
int CodeToHtmlConverter::highlightWord(int i, const LangData& data, StringView input, QString& output,
    ...
    if (i == 0 || (!input.at(i - 1).isLetterOrNumber() && input.at(i - 1) != QLatin1Char('_'))) {
        ...
        for (const auto& word : wordList) {
            if (word == input.mid(i, word.size())) {
                ...
                // word
                if (i + word.size() == input.length() || !input.at(i + word.size()).isLetterOrNumber()) {
...
QString CodeToHtmlConverter::xmlHighlighter(StringView input) const {
    ...
    for (int i = 0; i < textLen; ++i) {
        if (input.at(i) == QLatin1Char('<') && (i + 1 < textLen) && input.at(i + 1) != QLatin1Char('!')) {
 ...
 * @return
 */
QString CodeToHtmlConverter::cssHighlighter(StringView input, const LangData& types, const LangData& keywords) const {
    if (input.isEmpty()) return QLatin1String("");
    const auto textLen = input.length();
    QString output = QLatin1String("");
    // reserve extra space
...
QString CodeToHtmlConverter::setFormat(StringView str, CodeToHtmlConverter::Format format) {
    switch (format) {
        case Type:
            return QStringLiteral("<span class=\"code-type\">") % escapeString(str) % QStringLiteral("</span>");
        case Keyword:
            return QStringLiteral("<span class=\"code-keyword\">") % escapeString(str) % QStringLiteral("</span>");
        case Literal:
            return QStringLiteral("<span class=\"code-literal\">") % escapeString(str) % QStringLiteral("</span>");
        case String:
            return QStringLiteral("<span class=\"code-string\">") % escapeString(str) % QStringLiteral("</span>");
        case Builtin:
            return QStringLiteral("<span class=\"code-builtin\">") % escapeString(str) % QStringLiteral("</span>");
        case Other:
            return QStringLiteral("<span class=\"code-other\">") % escapeString(str) % QStringLiteral("</span>");
        case Comment:
            return QStringLiteral("<span class=\"code-comment\">") % escapeString(str) % QStringLiteral("</span>");
    }
