...
FakeVimProxy::FakeVimProxy(QWidget* widget, QObject* parent) : QObject(parent), m_widget(widget) {
    ...
    const QFile vimRcQONFile(QDir(homeDir).filePath(".vimrc.arcnotes"));

    // Try to load .vimrc.arcnotes or .vimrc files
    ...
    {
        auto h = [this](int beginLine, int endLine, QChar typedChar) { indentRegion(beginLine, endLine, typedChar); };
...
void FakeVimProxy::requestSetBlockSelection(const QTextCursor& tc) {
    ...
    QPalette pal = ed->parentWidget() != nullptr ? ed->parentWidget()->palette() : QApplication::palette();
