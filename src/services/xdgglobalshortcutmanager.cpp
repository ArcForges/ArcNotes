...
XdgGlobalShortcutManager::~XdgGlobalShortcutManager() {
    closeSession();
}

bool XdgGlobalShortcutManager::isAvailable() {
    QDBusInterface portal(PORTAL_SERVICE, PORTAL_PATH, QStringLiteral("org.freedesktop.DBus.Properties"),
    ...
    QDBusReply<QDBusVariant> reply = portal.call(
        QStringLiteral("Get"), QStringLiteral("org.freedesktop.portal.GlobalShortcuts"), QStringLiteral("version"));
...
void XdgGlobalShortcutManager::createSession() {
    ...
    QDBusMessage msg =
        QDBusMessage::createMethodCall(PORTAL_SERVICE, PORTAL_PATH, PORTAL_INTERFACE, QStringLiteral("CreateSession"));
    msg << options;
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [watcher](QDBusPendingCallWatcher* w) {
        if (w->isError()) {
            qWarning() << "XDG GlobalShortcuts CreateSession D-Bus error:" << w->error().message();
        }
        w->deleteLater();
    });
}
...
void XdgGlobalShortcutManager::closeSession() {
    ...
    QDBusMessage msg = QDBusMessage::createMethodCall(
        PORTAL_SERVICE, _sessionHandle, QStringLiteral("org.freedesktop.portal.Session"), QStringLiteral("Close"));
    bus.asyncCall(msg);
...
bool XdgGlobalShortcutManager::hasActiveSession() const {
    return !_sessionHandle.isEmpty();
}
...
void XdgGlobalShortcutManager::bindShortcuts() {
    ...
    QDBusMessage msg =
        QDBusMessage::createMethodCall(PORTAL_SERVICE, PORTAL_PATH, PORTAL_INTERFACE, QStringLiteral("BindShortcuts"));
    msg << QVariant::fromValue(QDBusObjectPath(_sessionHandle)) << QVariant::fromValue(shortcutsList)
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [watcher](QDBusPendingCallWatcher* w) {
        if (w->isError()) {
            qWarning() << "XDG GlobalShortcuts BindShortcuts D-Bus error:" << w->error().message();
        }
        w->deleteLater();
    });
}
...
void XdgGlobalShortcutManager::onShortcutActivated(QDBusObjectPath sessionHandle, QString shortcutId,
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    return QStringLiteral("arcnotes_%1").arg(QRandomGenerator::global()->generate());
#else
...
#endif
    token.remove(QLatin1Char('-'));
    return QStringLiteral("arcnotes_%1").arg(token);
#endif
}
...
QString XdgGlobalShortcutManager::computeRequestPath(const QString& handleToken) const {
    ...
    return QStringLiteral("/org/freedesktop/portal/desktop/request/%1/%2").arg(senderName, handleToken);
}
