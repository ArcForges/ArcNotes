...
ArcNotes source is AGPL-3.0-only. Third-party components retain their own copyright and licences. `Directory.Packages.props` and committed `packages.lock.json` files identify the exact resolved dependency graph; the SDK is pinned in `global.json`.
