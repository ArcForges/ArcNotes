...
namespace ArcForges.ArcNotes.Tests;
...
public sealed class ProvenancePolicyTests
{
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        public string Root { get; } = Path.Combine(Path.GetTempPath(), "arcnotes-provenance-test-" + Guid.NewGuid());
        ...
        public void Sync() => Write(ProvenancePolicy.Inventory, JsonSerializer.Serialize(new { schemaVersion = 1, repository = "ArcNotes", firstParty = Files().Where(path => !Reused.ContainsKey(path)).ToArray(), reused = Reused, artifacts = System.Array.Empty<string>() }));
        ...
        public void Dispose()
        {
            ...
            if (Path.GetDirectoryName(full) != Path.TrimEndingDirectorySeparator(Path.GetFullPath(Path.GetTempPath())) || !Path.GetFileName(full).StartsWith("arcnotes-provenance-test-", StringComparison.Ordinal))
