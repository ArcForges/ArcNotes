...
public sealed class CandidateTests
{
    ...
    {
        private readonly string _folder = Path.Combine(Path.GetTempPath(), "arcnotes-test-" + Guid.NewGuid());
        ...
        public CandidateFixture(string? unsafeArchive = null, bool success = true, string rid = "win-x64", string mode = "valid")
        {
            ...
            Archive = Path.Combine(_folder, $"arcnotes-{Version}-{rid}" + (rid.StartsWith("win-", StringComparison.Ordinal) ? ".zip" : ".tar.gz"));
