...
public static partial class Program
{
    ...
    private static string Executable(string rid) => Path.Combine(Stage(rid), rid.StartsWith("osx-", StringComparison.Ordinal)
        ? "ArcNotes.app/Contents/MacOS/ArcNotes" : rid.StartsWith("win-", StringComparison.Ordinal) ? "ArcNotes.exe" : "ArcNotes");
    ...
    private static async Task Prepare(string rid, string version)
    {
        ...
        var binaryRoot = rid.StartsWith("osx-", StringComparison.Ordinal) ? Path.Combine(stage, "ArcNotes.app/Contents/MacOS") : stage;
        ...
        if (rid.StartsWith("osx-", StringComparison.Ordinal))
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(stage, "ArcNotes.app/Contents/Info.plist"), $"""
                <?xml version="1.0" encoding="UTF-8"?>
                <!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
                <plist version="1.0"><dict>
                <key>CFBundleExecutable</key><string>ArcNotes</string>
                <key>CFBundleIdentifier</key><string>com.arcforges.arcnotes</string>
                <key>CFBundleName</key><string>ArcNotes</string>
                ...
                """);
            await Run("codesign", ["--force", "--deep", "--sign", "-", Path.Combine(stage, "ArcNotes.app")]);
            await Run("codesign", ["--verify", "--deep", "--strict", Path.Combine(stage, "ArcNotes.app")]);
        }
    }

    private static void WriteDependencyNotices(string stage)
    {
        using var assets = JsonDocument.Parse(File.ReadAllText("src/ArcForges.ArcNotes/obj/project.assets.json"));
    ...
    private static void Pack(string rid, string version, string commit)
    {
        ...
        var name = $"arcnotes-{version}-{rid}{extension}";
    ...
    public static string VerifyCandidate(string manifestPath, string version, string commit)
    {
        ...
        var expected = $"arcnotes-{version}-{manifest.Rid}" + (manifest.Rid.StartsWith("win-", StringComparison.Ordinal) ? ".zip" : ".tar.gz");
    ...
    private static void ValidateSmoke(JsonElement smoke, string rid, string version, string commit)
    {
        if (!smoke.GetProperty("success").GetBoolean() || !smoke.GetProperty("nativeAot").GetBoolean() ||
            ...
            smoke.GetProperty("version").GetString() != version || smoke.GetProperty("uiGreeting").GetString() != "Hello, ArcNotes!" ||
