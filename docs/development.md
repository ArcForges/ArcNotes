...
- `src/ArcForges.ArcNotes.Core`: owned published gRPC client and observable request state.
- `src/ArcForges.ArcNotes`: native Avalonia window with typed C# event wiring and explicit Generic Host lifetime. No reflection binding, dynamic XAML or WebView.
- `tests/ArcForges.ArcNotes.Tests`: deterministic loopback gRPC-Web, deadline/cancellation, UI state and release-integrity tests.
...
```sh
dotnet publish src/ArcForges.ArcNotes/ArcForges.ArcNotes.csproj -c Release -r win-x64 --no-restore -p:Version=0.1.0-ci.0.1 -p:SourceRevisionId=COMMIT -o artifacts/publish/win-x64
...
```sh
dotnet test --project tests/ArcForges.ArcNotes.Tests/ArcForges.ArcNotes.Tests.csproj -c Release --no-build --filter-class '*.TransportTests'
```
