...
Local validation covers file completeness, deterministic identity replacement, structured-file/reference consistency, unchanged third-party dependencies and licence hashes, workflow syntax and Git whitespace only. No local restore, build, test, native execution or live request is performed for ArcNotes. ArcChat's earlier results remain source-project evidence. Consult this repository's PR checks for its own hosted evidence; a copied pipeline is not proof of a product build or release.
