Native AOT ArcNotes Hello desktop client connected to `https://arcforges.com/api`.

Download the complete portable archive matching your OS and CPU, verify its SHA-256, and extract all files together. No .NET runtime installation or service token is needed. Windows: run `ArcNotes.exe`. Linux: run `./ArcNotes` in an X11/XWayland desktop with the documented system libraries. macOS: the archive contains `ArcNotes.app`.
