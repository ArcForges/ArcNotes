# ArcNotes development prerelease

Extract the complete Windows x64/ARM64 ZIP or Linux x64 tar.gz and keep its files together. No .NET runtime installation or service token is needed. Windows: run `ArcNotes.exe`. Linux: run `./ArcNotes` in an X11/XWayland desktop with the documented system libraries.
