...
5. The release job creates a draft `v0.1.0-ci.<run_number>.<run_attempt>`, uploads three archives, three checksum sidecars and `arcnotes-verification.tar.gz` containing build manifests, then exposes the prerelease. `Verify publication` requires the publishing job to succeed.
